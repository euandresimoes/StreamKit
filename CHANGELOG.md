# Streamlet 0.1.0-beta.2

[EN-US]

## What's new

- Fixed the Windows startup issue caused by an incompatible `better-sqlite3` native module.
- Added the Electron auto updater flow for checking, downloading and installing updates.
- Added localized release notes support for English, Portuguese and Spanish.
- Improved team tournaments, participant capture and provider integrations.

## Notes

- Windows releases are currently unsigned and may show a SmartScreen warning.
- Verify the published SHA-256 checksum before installing.
  [/EN-US]

[PT-BR]

## Novidades

- Corrigido o problema de inicialização do Windows causado por um módulo nativo incompatível do `better-sqlite3`.
- Adicionado o fluxo de atualização automática do Electron para verificar, baixar e instalar versões.
- Adicionado suporte a notas da versão em português, inglês e espanhol.
- Melhorias nos campeonatos por equipes, captura de participantes e integrações dos providers.

## Observações

- As versões do Windows ainda não possuem assinatura digital e podem exibir um aviso do SmartScreen.
- Confira o checksum SHA-256 publicado antes de instalar.
  [/PT-BR]

[ES]

## Novedades

- Corregido el problema de inicio de Windows causado por un módulo nativo incompatible de `better-sqlite3`.
- Añadido el flujo de actualización automática de Electron para comprobar, descargar e instalar versiones.
- Añadido soporte para notas de versión en español, inglés y portugués.
- Mejoras en los torneos por equipos, la captura de participantes y las integraciones de providers.

## Notas

- Las versiones de Windows todavía no están firmadas y pueden mostrar una advertencia de SmartScreen.
- Verifica el checksum SHA-256 publicado antes de instalar.
  [/ES]
