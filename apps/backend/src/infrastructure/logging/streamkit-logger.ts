...
export interface StreamKitLogger {
  log(level: LogLevel, message: string, context?: Record<string, unknown>): Promise<void>
  recent(limit?: number): Promise<string[]>
}
export const STREAMKIT_LOGGER = Symbol('STREAMKIT_LOGGER')

export class SilentStreamKitLogger implements StreamKitLogger {
...
export class RotatingFileLogger implements StreamKitLogger {
  public constructor(
