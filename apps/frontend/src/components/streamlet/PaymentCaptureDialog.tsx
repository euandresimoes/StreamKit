...
function draftKey(target: "giveaway" | "tournament", targetId: string) {
  return `streamlet:payment-capture-draft:${target}:${targetId}`;
}
      ...
      <DialogContent className="glass-panel max-h-[85vh] max-w-lg overflow-y-auto border-border-strong bg-popover/95">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <DollarSign className="size-5" /> {t("live.captureFromPayment")}
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-4 py-2">
          {!captures.connections.length || !currentConnection ? (
            <div className="rounded-xl border border-dashed border-border p-4 text-center text-xs text-muted-foreground">
              {t("live.connectChatBeforePaymentCapture")}
            </div>
          ) : (
            <>
              ...
              <div className="rounded-xl border border-border bg-card/45 p-3 text-xs text-muted-foreground">
                <p className="font-medium text-foreground">{t("live.captureConfirmedPayments")}</p>
                <p className="mt-1">{t("live.paymentCaptureDescription")}</p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <label className="space-y-1 text-xs text-muted-foreground">
                  ...
                  <Input
                    ...
                    placeholder={t("live.minimumAmountPlaceholder")}
                  />
                </label>
                <label className="space-y-1 text-xs text-muted-foreground">
                  {t("live.currency")}
                  <Input
                    ...
                    placeholder={t("live.currencyPlaceholder")}
                  />
                </label>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <BaseDateTimePicker
                  ...
                  ariaLabel={t("live.paymentCaptureStart")}
                  placeholder={t("live.startNowPlaceholder")}
                />
                <BaseDateTimePicker
                  value={draft.endsAt}
                  onChange={(value) => updateDraft({ endsAt: value })}
                  ariaLabel={t("live.paymentCaptureEnd")}
                  placeholder={t("live.noEndTimePlaceholder")}
                />
              </div>
              <div className="flex items-center justify-between rounded-md border border-border bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
                <span>
                  {t("live.paymentParticipantsCaptured", {
                    count: currentRule?.capturedCount ?? 0,
                  })}
                </span>
                <span>
                  {t("live.chatMatching", { provider: currentConnection.provider.toUpperCase() })}
                </span>
              </div>
              <Button
                className="w-full"
              ...
              >
                <Radio />{" "}
                {isCapturing ? t("live.stopCapturingPayments") : t("live.startCapturingPayments")} (
