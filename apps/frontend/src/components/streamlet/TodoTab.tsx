import type { TodoCard, TodoColumn, TodoPriority, UpdateCardRequest } from "@streamlet/contracts";
import {
  useEffect,
  useMemo,
  useRef,
...
export function TodoTab() {
  ...
  const TODO_TEXT = {
    addCard: t("todo.addCard"),
    addColumn: t("todo.addColumn"),
    boardHint: t("todo.boardHint"),
    cardDetails: t("todo.cardDetails"),
    cardTitle: t("todo.cardTitle"),
    columnTitle: t("todo.columnTitle"),
    description: t("todo.description"),
    notes: t("todo.notes"),
    priority: t("todo.priority"),
    searchWorkspace: t("todo.searchWorkspace"),
    templates: t("todo.templates"),
    workspaces: t("todo.workspaces"),
  };
  ...
  return (
    <div className="flex h-full min-h-0">
            ...
            >
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent>
                  ...
                  >
                    <Pin /> {workspace.isPinned ? t("todo.unpin") : t("todo.pin")}
                  </DropdownMenuItem>
                  <DropdownMenuItem
      ...
      <main className="flex min-w-0 flex-1 flex-col rounded-2xl">
        <header className="flex items-center gap-3 border-b border-border px-5 py-2">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <button
                aria-label={t("todo.workspaceColor")}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent>
                        ...
                        >
                          <Star /> {column.isPinned ? t("todo.unpin") : t("todo.pin")}
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          className="text-destructive"
                          onSelect={() =>
      ...
      <BaseConfirmDialog
        ...
        title={
          pendingDelete?.kind === "card"
            ? t("todo.deleteCardTitle")
            : pendingDelete?.kind === "column"
              ? t("todo.deleteColumnTitle")
              : t("todo.deleteWorkspaceTitle")
        }
        description={t("todo.permanentlyRemoved", { name: pendingDelete?.name ?? "" })}
        onConfirm={async () => {
      ...
      <BaseModal
        ...
        title={t("todo.globalTemplates")}
        description={t("todo.saveTemplates")}
      >
        <div className="space-y-3">
          <div className="flex gap-2">
            <Input
              ...
              placeholder={t("todo.templateName")}
            />
            <Button
    ...
    <aside className=" flex w-[320px] shrink-0 flex-col rounded-2xl border-l border-border p-4">
      ...
      <div className="space-y-3">
        ...
        <Select value={priority} onValueChange={(value) => setPriority(value as TodoPriority)}>
          <SelectTrigger aria-label={t("todo.priority")}>
          ...
          <SelectContent>
            {PRIORITIES.map((item) => (
              <SelectItem key={item} value={item}>
                {t(
                  `todo.priority${item.charAt(0).toUpperCase()}${item.slice(1)}` as
                    | "todo.priorityLow"
                    | "todo.priorityNormal"
                    | "todo.priorityHigh"
                    | "todo.priorityUrgent",
