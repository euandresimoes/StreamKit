# Streamlet — Especificação de Produto, Arquitetura e Execução

> Documento mestre para orientar o planejamento, a implementação, os testes e o lançamento do MVP do Streamlet.
...
O **Streamlet** será uma toolbox desktop para streamers. O aplicativo reunirá ferramentas de organização, torneios, sorteios e integrações em uma única experiência, mantendo todos os dados localmente no computador do usuário.

A proposta não é criar apenas uma roleta ou um gerenciador de tarefas. O Streamlet deverá funcionar como um **companion app para transmissões**, capaz de acompanhar o streamer antes, durante e depois da live.
...
1. instalar e abrir o Streamlet;
...
2. o Streamlet recebe um evento válido do LivePix;
...
O visual pode ser inspirado no formato, mas deve utilizar identidade, assets e sons próprios do Streamlet.
...
Para o MVP, usar **um banco principal `streamlet.db`** é mais seguro do que um `.db` separado por módulo. Isso permite transações consistentes, migrações únicas e backups simples. Os dados de TODO, Games, Giveaway e configurações ficam em tabelas separadas dentro do mesmo arquivo.
...
Depois do MVP, o Streamlet poderá evoluir para uma plataforma completa de operação de lives:
...
O primeiro marco real é simples: **um streamer instalar, usar o Streamlet em uma live, fechar o aplicativo e voltar depois com todo o seu trabalho preservado.**
